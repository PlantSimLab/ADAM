    	    ...
    	    when (/(SDDS)/) {
	          ...
	          `mkdir -p $cwd_sdds/../../htdocs/no-ssl/files`;
	            ...
	            if ($upload_file_pm) {
		            ...
		            system("/usr/bin/perl SDDS.pl -t $filename_tt -p $filename_pm -i $initialState -n $interestingNodes -s $num_states -f $flag4ss -m $flag4tm -g $plot_file -h $histogram_file -x $tm_file");
	            } 
	            else {
                    say("usr/bin/perl SDDS.pl -t $filename_tt -i $initialState -n $interestingNodes -s $num_states -f $flag4ss -m $flag4tm -g $plot_file -h $histogram_file -x $tm_file") if $DEBUG;
		            system("usr/bin/perl SDDS.pl -t $filename_tt -i $initialState -n $interestingNodes -s $num_states -f $flag4ss -m $flag4tm -g $plot_file -h $histogram_file -x $tm_file");
	            }
            } # end of /SDDS/
