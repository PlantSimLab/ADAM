...
S:=newRing(R,MonomialOrder=>Lex); RS:=apply(m,i->(R_i=>S_i));
I=ideal apply(#I,i->sub(I_i,RS));I=(ideal gens gb I)_*;
RS=apply(m,i->(S_i=>R_i));apply(#I,i->sub(I_i,RS))  );
...
T3=method();
T3(List,Ring):=(POLY,R)->(P:=GBFP0 (POLY-gens R,R);
     if not P=!=null then return {};
     p:=char R;
     SOL:={};
     QSOL:={order(P,R)}; 
...
T4(List,Ring):=(POLY,R)->(
   ...
   LP0:=LP1:=LP2:={}; 
   if #P_2==0 then LP2={(P_0,P_1,P_4,P_5)} 
   else LP1={(P_0,P_1,P_2,P_3,P_5)};
